...
function(input, output, session) {
  ...
  toListen <- reactive({
    list(input$tabs == "stage",input$stage)
  })
  ...
  observeEvent(toListen(), {
    ...
    if(input$stage == "all"){
        data <- data_stage_clean_points()
    }
    if(input$stage == "M1"){
        data <- data_stage_clean_points()[data_stage_clean_points()$Stage == '1',]
    }
    if(input$stage == "alternance"){
        data <- data_stage_clean_points()[data_stage_clean_points()$Stage == '2',]
    }
    if(input$stage == "M2"){
      data <- data_stage_clean_points()[data_stage_clean_points()$Stage == '3',]
    }
    
    # remove duplicate labs ?

    getColor <- function(data) {
      sapply(data$Stage, function(Stage) {
        if(Stage == 1) {
          "green"
        } else if(Stage == 2) {
          "orange"
    ...
    output$mymap <- renderLeaflet({
      leaflet(data) %>%
        ...
        addLegend("bottomright", colors = c("green", "orange", "red"), labels = c("Stage M1", "Alternance", "Stage M2"),
                  opacity = 1)
    })
  })
